            ...
            {thinking ? (
              ...
              <div className="px-4 py-3 space-y-2">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {liveDecision && displayDecision === liveDecision && (
                    <span className="text-xs text-money font-mono" title="re-evaluated against current persona + market">LIVE</span>
                  )}
                  {displayDecision.decisionSource && (
                    <span
                      className={`chip ${displayDecision.decisionSource === 'FLOCK' ? 'chip-money' : 'chip-ghost'}`}
                      title={displayDecision.decisionSource === 'FLOCK' ? 'generated by FLock LLM' : 'rule engine fallback'}
                    >
                      {displayDecision.decisionSource === 'FLOCK' ? 'FLock LLM' : 'RULES'}
                    </span>
                  )}
